...
| 模式 | 说明 |
| --- | --- |
| 我 vs 模型 | 你执黑先手；对手从模型下拉里选（默认 = 本会话当前模型），常用 DeepSeek / Kimi 等已在列表里 |
...
- **兜底**：模型没给出合法坐标时，贴着已有棋子自动落一个，并在状态行说明原因（不会卡死整局）。
...
| POST | `/gomoku/move` | 请求 `{provider, model, name, side(1黑/2白), size, cells(长度 size*size), history:[{r,c,side}]}`；响应 `{r, c, fallback, from:'text'\|'reasoning'\|'fallback', text, reasoning, finishKind, usage, ms, name}`；失败返回 500 `{ok:false,error}` |
...
| 文件 | 作用 |
| --- | --- |
| `index.js` | host 半：模型目录 + 走棋（`inject: ['webServer','llm']`），两条路由 |
| `client.js` | client 半：`conversation.view` 标签页 + `shell.overlay` 浮窗 |
| `cordis.patch.yml` | 挂载声明（bundle patch），插入 id `gomoku` |
| `tools/verify-gomoku.mjs` | 离线自检：胜负判定、坐标解析、兜底选点（不开浏览器） |
